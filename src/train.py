...
def main_worker(gpu_idx, configs):
	...
	if logger is not None:
		logger.info(f'number of batches in training set: {len(train_dataloader)}')
	...
	for epoch in range(configs.start_epoch, configs.num_epochs + 1):
		if logger is not None:
			logger.info('*-' * 40)
			logger.info('=' * 35 +f'{epoch}/{configs.num_epochs}' + '=' * 35)
			logger.info('*-' * 40)
			logger.info(f'>>> Epoch: [{epoch}/{configs.num_epochs}]')
