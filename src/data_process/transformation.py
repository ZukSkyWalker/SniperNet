import numpy as np
import cfg.kitti_config as kitti_cfg
...
def camera_to_lidar(x, y, z, V2C=None, R0=None, P2=None):
  ...
  if V2C is None or R0 is None:
    p = kitti_cfg.Tr_velo_to_cam_inv @ kitti_cfg.R0_inv @ p
  else:
    p = R0 @ V2C @ p
  return tuple(p[:3])
...
def lidar_to_camera(x, y, z, V2C=None, R0=None, P2=None):
  p = np.array([x, y, z, 1])
  if V2C is None or R0 is None:
    p = kitti_cfg.R0 @ kitti_cfg.Tr_velo_to_cam @ p
  else:
    p = R0 @ V2C @ p
  return tuple(p[0:3])
...
def camera_to_lidar_point(points):
  ...
  points = kitti_cfg.R0_inv @ points
  points = (kitti_cfg.Tr_velo_to_cam_inv @ points).T  # (4, N) -> (N, 4)

  return points[:, 0:3].reshape(-1, 3)
...
def lidar_to_camera_point(points, V2C=None, R0=None):
  ...
  if V2C is None or R0 is None:
      points = kitti_cfg.Tr_velo_to_cam @ points
      points = (kitti_cfg.R0 @ points).T
  else:
...
def camera_to_lidar_box(boxes, V2C=None, R0=None, P2=None):
  ...
  for box in boxes:
    ...
    ret.append([x, y, z, h, w, l, -ry - np.pi / 2])
...
def lidar_to_camera_box(boxes, V2C=None, R0=None, P2=None):
  ...
  for box in boxes:
    ...
    ret.append([x, y, z, h, w, l, -rz - np.pi / 2])
  return np.array(ret).reshape(-1, 7)
...
def inverse_rigid_trans(Tr):
  ...
  inv_Tr[0:3, 3] = -(Tr[0:3, 0:3]).T @ Tr[0:3, 3]
  return inv_Tr
...
class Cutout(object):
  ...
  def __init__(self, n_holes, ratio, fill_value=0., p=1.0):
    ...
    self.ratio = ratio
    assert 0. <= fill_value <= 1., f"the fill value should be in a range of 0 to 1, got {fill_value:.2f}"
